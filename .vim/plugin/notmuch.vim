...
EOF
	...
	"let cmdtxt = g:notmuch_sendmail . ' -a baylibre -t -X ""  < ' . fname
	echo cmdtxt
...
function! s:search_tag(intags)
	...
	" FIXME, make the below change configurable
	"norm j
...
function! s:search_show_thread_mutt(thread_id)
ruby << EOF
	...
	mbox = File.expand_path('~/.notmuch/vim_mbox')
	cmd = VIM::evaluate('g:notmuch_reader') % mbox
	#system "notmuch show --format=mbox thread:#{thread_id} > #{mbox} && #{cmd}"
	#system "notmuch show --format=mbox id:#{m.message_id} > #{mbox} && #{cmd}"
	#system "notmuch show --format=mbox thread:#{m.thread_id} > #{mbox} && #{cmd}"
	system "notmuch show --format=mbox #{m} > #{mbox} && #{cmd} ; rm #{mbox}"
EOF
endfunction
...
ruby << EOF
	...
	when 2; $cur_filter = $cur_search
		VIM::command("call s:show('#{id}')")
	when 3; VIM::command("call s:search_show_thread_mutt('#{id}')")
	end
EOF
endfunction
